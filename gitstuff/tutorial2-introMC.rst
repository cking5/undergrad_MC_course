...
For any general 2D lattice where coupling along rows and along columns are equal, the *T_{c}* is given by:

..math::

	T_{c} = frac{2}{\ln(1+\sqrt{2})} \approx 2.269
	
Does your estimation of *T_{c}* agree with that predicted by the above equation? Account for any observed discrepancies.
...
First, take the REVCON from one of your simulations where the *T* > *T_{c}*, copy it into a new directory and then rename it 'CONFIG'.  Also copy the CONTROL and FIELD files into this directory and change the temperature to \sim 10^{-3} K.  Then run the simulation.  
...
Does this agree with magnetic behaviour predicted by the Ising model? How does this compare with the time evolution at *T* > *T_{c}*?
